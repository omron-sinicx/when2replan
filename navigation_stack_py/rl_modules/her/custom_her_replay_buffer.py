...
class CustomHerReplayBuffer(DictReplayBuffer):
    ...
    def __init__(
        ...
        if not self.online_sampling:
            raise NotImplementedError("offline sampling is not supported")
    ...
    def _sample_online(
        ...
        valid_inds = [
            np.arange(self.buffer_size)[is_valid[:, env_idx]]
            for env_idx in range(self.n_envs)
        ]
        ...
        virtual_data = self._get_virtual_samples_with_averaged_reward(
            virtual_batch_inds, virtual_env_indices, env
        )
        # virtual_data = self._get_virtual_samples_custom(virtual_batch_inds, virtual_env_indices, env)
    ...
    def _get_virtual_samples_with_averaged_reward(
        self,
        batch_inds: np.ndarray,
        env_indices: np.ndarray,
        env: Optional[VecNormalize] = None,
        ...
        new_goals = self._sample_goals(
            batch_inds, env_indices, self.goal_selection_strategy
        )
        ...
        # NEW
        # make reward average over samples in batch
        num_average = 5
        random_inds = np.random.randint(
            0, rewards.shape[0], size=(rewards.shape[0], num_average)
        )
        averaged_rewards = np.mean(rewards[random_inds], axis=1)
        ...
        return DictReplayBufferSamples(
            ...
            rewards=self.to_torch(
                self._normalize_reward(averaged_rewards.reshape(-1, 1), env)
            ),
        )
    ...
    def _get_final_indices(
        ...
        final_indices = batch_ep_start + batch_ep_length - 1

        return final_indices % self.buffer_size

    # TODO: WIP
    def _get_virtual_samples_custom(
        self,
        batch_inds: np.ndarray,
        env_indices: np.ndarray,
        env: Optional[VecNormalize] = None,
        ...
        # Get final indices of the episodes
        final_indices = self._get_final_indices(batch_inds, env_indices)

        # separate timeout episodes and other episodes
        # TODO: 本当に分離出来ているかは要検討
        # timeoutsがTrueのやつがtimeoutしたものだと仮定している
        timeout_indices = np.where(self.timeouts[final_indices, env_indices] == 1)[0]
        timeout_batch_inds = batch_inds[timeout_indices]
        not_timeout_batch_inds = np.delete(batch_inds, timeout_indices)
        assert len(timeout_batch_inds) + len(not_timeout_batch_inds) == len(batch_inds)

        # timeout episodes -> convert to virtual samples
        timeout_obs = {
            key: obs[timeout_batch_inds, env_indices, :]
            for key, obs in self.observations.items()
        }
        timeout_next_obs = {
            key: obs[timeout_batch_inds, env_indices, :]
        ...
        timeout_infos = copy.deepcopy(self.infos[timeout_batch_inds, env_indices])
        timeout_new_goals = self._sample_goals(
            timeout_batch_inds, env_indices, GoalSelectionStrategy.FINAL
        )
        timeout_obs["desired_goal"] = timeout_new_goals
        timeout_next_obs["desired_goal"] = timeout_new_goals
        for info in timeout_infos:
            info.pop("is_success", None)
            info.pop("TimeLimit.truncated", None)
        timeout_rewards = self.env.env_method(
            "compute_reward",
            # here we use the new desired goal
            timeout_obs["desired_goal"],
            # the new state depends on the previous state and action
            # s_{t+1} = f(s_t, a_t)
            # so the next achieved_goal depends also on the previous state and action
            # because we are in a GoalEnv:
            # r_t = reward(s_t, a_t) = reward(next_achieved_goal, desired_goal)
            # therefore we have to use next_obs["achived_goal"] and not obs["achived_goal"]
            timeout_next_obs["achieved_goal"],
            timeout_infos,
            # we use the method of the first environment assuming that all environments are identical.
            indices=[0],
        )
        timeout_rewards = timeout_rewards[0].astype(
            np.float32
        )  # env_method returns a list containing one element
        timeout_obs = self._normalize_obs(timeout_obs)
        timeout_next_obs = self._normalize_obs(timeout_next_obs)

        # TODO: ↑の処理は不要なかも. finalでやれば, そもそもtimeoutしたepisodeは含まれなくなる

        # timeoutとnot_timeoutを結合

        # rewardをいくつかのサンプルの平均にする
