...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        self.prioritized_replay_kwargs = prioritized_replay_kwargs
    ...
    def _setup_model(self) -> None:
        ...
        if self.replay_buffer is None:
            ...
            if issubclass(self.replay_buffer_class, HerReplayBuffer):
                ...
                args = (self.env,)
            if issubclass(self.replay_buffer_class, PrioritizedReplayBuffer):
                if self.prioritized_replay_kwargs is None:
                    pass
                else:
                    alpha = self.prioritized_replay_kwargs.get("alpha", 0.6)
                    initial_beta = self.prioritized_replay_kwargs.get(
                        "initial_beta", 0.4
                    )
                    beta_increment = self.prioritized_replay_kwargs.get(
                        "beta_increment", 0.001
                    )
                    args = (alpha, initial_beta, beta_increment)
    ...
    def load_replay_buffer(
        ...
        if isinstance(self.replay_buffer, HerReplayBuffer) or isinstance(
            self.replay_buffer, CustomHerReplayBuffer
        ):
            assert (
                self.env is not None
    ...
    def _dump_logs(self) -> None:
        ...
        if len(self.ep_info_buffer) > 0 and len(self.ep_info_buffer[0]) > 0:
            ...
            if self.num_timesteps > self.learning_starts:
                self.logger.record(
                    "rollout/ep_rew_mean_from_learning_start",
                    safe_mean([ep_info["r"] for ep_info in self.ep_info_buffer]),
                )
