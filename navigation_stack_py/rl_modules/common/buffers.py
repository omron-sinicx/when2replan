...
from navigation_stack_py.rl_modules.common.sum_tree import SumTree
...
class PrioritizedReplayBuffer(BaseBuffer):
    ...
    :param beta: To what degree to use importance weights [0, 1]
        (0 - no corrections, 1 - full correction)
    :param beta_increment_per_sampling: Value to increment beta with at each sampling
    :param epsilon: Small number to avoid zero priority
    :param device: PyTorch device
    :param n_envs: Number of parallel environments
    :param optimize_memory_usage: Enable a memory efficient variant
        of the replay buffer which reduces by almost a factor two the memory used,
    ...
    def __init__(
        ...
        beta_increment_per_sampling: float = 0.00001,
        epsilon: float = 1e-6,
        device: Union[th.device, str] = "auto",
        n_envs: int = 1,
        optimize_memory_usage: bool = False,
        ...
        # sum-tree structure for fast sampling
        self.sum_tree = SumTree(self.buffer_size, epsilon)

        # Prioritized Experience Replay (PER) parameters
        if not 0 <= alpha <= 1:
            raise ValueError("alpha parameter must be in [0, 1]")
        if not 0 <= initial_beta <= 1:
    ...
    def add(
        ...
        # because every transition should be sampled at least once
        self.sum_tree.add(self.max_priority, self.pos)

    def sample(
        self, batch_size: int, env: Optional[VecNormalize] = None
    ) -> ReplayBufferSamples:
        """
        Sample elements from the replay buffer considering priorities.
        Custom sampling when using memory efficient variant,
        as we should not sample the element with index `self.pos`
        See https://github.com/DLR-RM/stable-baselines3/pull/28#issuecomment-637559274
        ...
        :return:
        """
        sample_indices = np.zeros(batch_size, dtype=np.int32)
        tree_indices = np.zeros(batch_size, dtype=np.int32)
        priorities = np.zeros(batch_size, dtype=np.float32)

        # linear increase beta for prioritized sampling
        ...
        weights = (self.sum_tree.n_entries * probs) ** -self.beta
        ...
        return self._get_samples(sample_indices, env=env), weights, tree_indices
    ...
    def update_priorities(
        ...
        :param data_indices: indices of sampled transitions
        :param priorities: new priorities that is importance of the transitions, larger value means more important
        """
