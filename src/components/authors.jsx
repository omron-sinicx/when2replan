    ...
    return (
      <div>
        <div className="uk-flex uk-flex-center uk-text-primary">
          {this.props.authors.map((author)=>{ return <span className="uk-margin-left"><a target="_blank" className="uk-link-toggle" href={author.url}>{author.name}</a><sup>{author.affiliation.join(',')}</sup></span> })}
        </div>
